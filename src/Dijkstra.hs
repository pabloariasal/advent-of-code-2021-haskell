...
step s@DState{..} =
  let
      (currentNode, newQueue) = Q.deleteFindMin dQueue
  in
    if currentNode == dEnd then
      s {dQueue = Q.empty}
    else
      foldr (update currentNode) s {dQueue = newQueue } $ dEdgesFunc currentNode
...
update current (next, distance) s@DState{..} =
  ...
  where
    addNeighbor = s {dQueue = Q.insert next dQueue,
